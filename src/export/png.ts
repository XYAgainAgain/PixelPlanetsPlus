...
export const assertPngAdmission = (request: Parameters<ExportRunner>[0], options: PngExportRunOptions): void => {
    const result = preflightRenderRequest(request, options.preflightLimits ?? runtimeLimits())
...
export const exportCompositePng = async (
    ...
    try {
        ...
        return {
            files: [{ filename: `${request.recipe.celestialType}-${request.recipe.seed}.png`, mediaType: 'image/png', data }],
            warnings: options.preflightLimits ? [] : [`This device limits exports to ${CONSERVATIVE_TEXTURE_LIMIT} pixels per side.`],
        }
    } finally {
        session.dispose()
    }
